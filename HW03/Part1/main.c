...
int main (int argc, char **argv) {
  ...
  if (rank == 1) {
    ...
    MPI_Send(message, Nmessages, MPI_UNSIGNED, 0, 1, MPI_COMM_WORLD);
    MPI_Send(a, Nmessages, MPI_UNSIGNED, 0, 2, MPI_COMM_WORLD);
  }
  ...
  if (rank == 0) {
    ...
    MPI_Recv(message, Nmessages, MPI_UNSIGNED, 1, 1, MPI_COMM_WORLD, &mStat);
    MPI_Recv(a, Nmessages, MPI_UNSIGNED, 1, 2, MPI_COMM_WORLD, &aStat);
